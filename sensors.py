...
#Select the appropiate sensor if requested
...
#Select appropiate orientation value if requested
if args.r:
    rval=orientation['roll']
    if args.r == 'deg':
        rval=rval*180/math.pi
    data_out.append({'roll':rval,'unit':args.r})
if args.p:
    pval=orientation['roll']
    if args.p == 'deg':
        pval=pval*180/math.pi
    data_out.append({'pitch':pval,'unit':args.p})
if args.y:
    yval=orientation['roll']
    if args.y == 'deg':
        yval=yval*180/math.pi
    data_out.append({'yaw':yval,'unit':args.y})
